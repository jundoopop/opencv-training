...
sketchbook = np.array([[il.bi_inter(i // ratio, j // ratio, img) for i in range(dsize[0])] for j in range(dsize[1])])
...
while cv.waitKey(0) != ord("q"):
    ...
    # Custom bilinear interpolation formula
    cv.imshow("Custom bilinear interpolation",
              sketchbook.astype(np.uint8))
